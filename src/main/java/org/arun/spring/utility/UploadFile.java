...
public class UploadFile {
	...
	public static String uploadMusic(CommonsMultipartFile multipartFile) throws IOException {
		String fileName = null;
		String aTempDir = null;
		String fileLocation = null;
		try {
			aTempDir = System.getProperty("catalina.base") + java.io.File.separator + "webapps" + java.io.File.separator
					+ "Files" + java.io.File.separator + "music";
		...
		return "/Files/music/" + fileName;

	}

	public static String uploadArticleAttachment(CommonsMultipartFile multipartFile, String title) throws IOException {
		String fileName = null;
		String aTempDir = null;
		String fileLocation = null;
		try {
			aTempDir = System.getProperty("catalina.base") + java.io.File.separator + "webapps" + java.io.File.separator
					+ "Files" + java.io.File.separator + "article";

			// File code
			String nameOfFIle;
			Calendar calendar = Calendar.getInstance();
			Date date = calendar.getTime();
			nameOfFIle = title + date.getTime();
			try {
				String[] originalFileName = multipartFile.getOriginalFilename().split("\\.");
				fileName = originalFileName[0] + nameOfFIle + "." + originalFileName[originalFileName.length - 1];
			} catch (Exception e) {
